...
1. Identify a quantitative outcome. 
    - For purposes of this project, we will require your quantitative outcome to contain more than 20 unique values.
2. Identify a key predictor (which may be either quantitative or categorical.)
    - If the key predictor is categorical, it must have 2-6 categories, and each category must contain at least 30 observations.
3. Identify 3-8 other predictors of your outcome (demonstrating that at least one of the "other" predictors is multi-categorical with 3-6 categories.)
...
5. Steps 1-3 will yield a set of 6-11 variables (an outcome, a key predictor, 3-8 other predictors, and a subject identifier). Use those selections to create your analytic data set.
    - You must have at least 250 and no more than 10000 observations with complete data on all 6-11 variables included in your analytic data set. No other variables should be included in your analytic data set.
    - Once you have these data in hand, you should be able to complete the [Study 2 Data Plan](your5.html), which is due on 2020-11-19 at Noon.
...
8. Partition the clean data into a model development (also called a model training) sample (70-80% of the data) and a model testing (also called a model validation) sample (the remaining 20-30%).
...
10. Produce two competitive models for predicting your outcome using your data that provide evidence regarding your research question.
    ...
    - Your other model should be a well-motivated subset of your full model, that at least includes the key predictor.
