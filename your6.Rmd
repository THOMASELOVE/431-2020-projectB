...
3. Cleaning the Data 
    ...
    - If you are working with NHANES data, do all of your cleaning after the merge. I also suggest applying **clean_names** after merging, but I wouldn't otherwise change the variable names. If you decide to change the names, that's OK, but then specify what their original names were in the codebook below.
    - If any of the variables have responses like "don't know" or "did not respond" or "unknown" I would drop those cases in the cleaning process, treating those as missing values.
...
5. My Final Variables
    - Subsection 5.1 should be called Codebook. In it, provide an attractive codebook specifying the names and descriptions of each of your 6-11 variables in your final data set. Specify incredibly clearly which is the outcome, and which is the key predictor. Don't make me guess.
        - The Codebook lists all of the variables you will use in your analyses (plus the subject identifying code) and no other variables. 
        - Each variable should have its name (and its original name if you've changed it from the initial download) and a description of the variable where you specify its type as quantitative, binary or multi-categorical. 
        - For categorical variables, list the levels in the codebook. 
        - Be sure to indicate which variable is your outcome by including **outcome** in the description of that variable in your codebook.
    - Subsection 5.2 should be called Numerical Data Description, and should present the results of  **Hmisc::describe** or **mosaic::favstats** or something similar for all variables of interest (e.g. leave out the subject identifier)
